...
public class Send {
	 ...
	 * @throws IOException
	 */
	private static void sendByte(byte[] message)
			throws IOException {
	 ...
	 * NOTHALT</BR>
	 * OPCODE: 0x03</BR>
	 * Nothalt wird an erste Stelle der Warteliste gesetzt und somit vor allen
	 * anderen potentiell existierenden Befehlen in der Liste priorisiert!</BR>
	 * Damit eventuell darauf folgende Geschwindigkeitsaenderungen den Nothalt 
	 * nicht wieder "ueberschreiben" bzw. unnuetz machen, werden alle OPCODE 
	 * 0x24 Auftraege aus der Warteschlange entfernt. 
	 */
	...
	/**
	 * Sendet eine Nachricht an den RMX-Server mit folgendem Befehlsinhalt:</BR>
	 * Auslesen der Lok-DB aus der PC-Zentrale</BR>
	 * OPCODE: 0x08
	 */
	...
	/**
	 * Sendet eine Nachricht an den RMX-Server mit folgendem Befehlsinhalt:</BR>
	 * Initialisierung der Verbindung</BR>
	 * OPCODE: 0x03
	 */
	...
	/**
	 * Sendet eine Nachricht an den RMX-Server mit folgendem Befehlsinhalt:</BR>
	 * Anfrage einer Positivquittung</BR>
	 * OPCODE: 0x00
	 */
	 ...
	 * Statusabfrage</BR>
	 * OPCODE: 0x04</BR>
	 * Da der PC-Server in der aktuellen Version automatisch eine Statusinformation 
	 * an den Client sendet, sollte es eigentlich nicht noetig sein eine manuelle
	 * Statusabfrage anzustossen. Das Paket bietet diese Funktion jedoch der 
	 * Vollstaendigkeit halber an, da das RMX-Protokoll diese Moeglichkeit vorsieht. 
	 */
	...
	/**
	 * Sendet eine Nachricht an den RMX-Server mit folgendem Befehlsinhalt:</BR>
	 * Wert in RMX1-Adresse schreiben</BR>
	 * OPCODE: 0x05
	 * 
	 * @param address - byte - Die Adresse, in welche geschrieben werden soll
	 * @param value - byte - Der Wert, welcher geschrieben werden soll
	 */
	public static void send0x05(byte address, byte value) {
		...
		outgoingList.add(new byte[] { Connection.HEADBYTE, 0x06, 0x05, 0x01, address, value });
	}
	
	/**
	 * Sendet eine Nachricht an den RMX-Server mit folgendem Befehlsinhalt:</BR>
	 * Wert in RMX-Adresse lesen</BR>
	 * OPCODE: 0x06
	 * 
	 * @param rmx - int - Der RMX-Bus auf dem gelesen werden soll
	 * @param address - byte - Die Adresse aus welcher gelesen werden soll
	 */
	public static void send0x06(int rmx, byte address) {
		...
		outgoingList.add(new byte[] { Connection.HEADBYTE, 0x05, 0x06, (byte) rmx, address });
	}
	
	/**
	 * Sendet eine Nachricht an den RMX-Server mit folgendem Befehlsinhalt:</BR>
	 * Eingabe Lokinformation in Zentrale</BR>
	 * OPCODE: 0x08</BR></BR>
	 * 
     * Die Funktion der Lokeingabe wird wohl nicht in der App
	 * unterstuetzt. Das Paket bietet diese Funktion jedoch der Vollstaendigkeit
	 * halber an, da das RMX-Protokoll diese Moeglichkeit vorsieht. </BR></BR>
	 * 
	 * Spezielle OPMODES: (andere siehe Spezifikation)</BR>
	 * 31 - 
	 * Löschen einer Lok über Loknummer: Angabe der alphanumerische
	 * Zeichen nicht erlaubt. <COUNT> = 7. Wird beim Löschen die
 	 * Loknummer 0000 eingegeben, wird die gesamte Datenbank gelöscht.</BR>
	 * 32 - 
	 * Lesen eines Datensatzes über Loknummer: Angabe der alphanumerische
	 * Zeichen nicht erlaubt. <COUNT> = 7.</BR>
	 * 
	 * @param trainNumber - int 
	 * @param opmode - int - Vom Server akzeptierte Werte: 2, 3, 5, 6, 7, 9, 10, 
	 * 12, 13, 15, 16, 17, 31, 32
	 * @param name - String - Zu lange Namen, die vom Server nicht angenommen  
	 * werden, werden abgeschnitten. </BR>Soll kein Name an den Server uebermittelt
	 * werden, einfach name = "" uebergeben. 
	 * */
	public static void send0x08(int trainNumber, int opmode, String name) {
		...
		// temp als Hilfsvariable, falls der angegebene Name 20 Bytes ueberschreitet
		...
		// erster Array-Teil: (wird wegen der second.length erst als zweites bearbeitet)
		byte[] first = new byte[] 
				//             <0x7c>  <COUNT>
		...
		switch (opmode) {
		case 2: // Hex: 0x02
			// SX1: kurze Adresse, Steuerung über Loknummer
		...
		case 3: // Hex: 0x03
			...
			break;
		case 5: // Hex: 0x05
			// SX1: kurze Adresse, Steuerung über Loknummer mit
		...
		case 7: // Hex: 0x07
			...
			break;
		case 9: // Hex: 0x09
			// DCC: 14 Fahrstufen, kurze Adresse, Steuerung über Loknummer
		...
		case 10: // Hex: 0x0a
			...
			break;
		case 12: // Hex: 0x0c
			// DCC: 28 Fahrstufen, kurze Adresse, Steuerung über Loknummer
		...
		case 13: // Hex: 0x0d
			...
			break;
		case 15: // Hex: 0x0f
			// DCC: 126 Fahrstufen, kurze Adresse, Steuerung über Loknummer
		...
		case 31: // Hex: 0x1f
			 ...
			 * Löschen einer Lok über Loknummer: Angabe der alphanumerische
			 * Zeichen nicht erlaubt. <COUNT> = 7. Wird beim Löschen die
			 * Loknummer 0000 eingegeben, wird die gesamte Datenbank gelöscht.
			 */
			// Loknummer schreiben
		...
		case 32: // Hex: 0x20
			/*
			 * Lesen eines Datensatzes über Loknummer: Angabe der
	...
	/**
	 * Sendet eine Nachricht an den RMX-Server mit folgendem Befehlsinhalt:</BR>
	 * Loksteuerung Geschwindigkeit</BR>
	 * OPCODE: 0x24
	 * 
	 * @param trainNumber - int - Nummer der Lok, welche gesteuert werden soll
	 * @param runningNotch - int - Die Fahrstufe, welche eingestellt werden soll
	 * @param direction - byte - Die Richtung, in welche sich die Lok bewegen soll.</BR>
	 * Bit 0 = 0: Fahrtrichtung vorwärts</BR>
	 * Bit 0 = 1: Fahrtrichtung rückwärts
	 * */
	public static void send0x24(int trainNumber, int runningNotch, byte direction) {
	...
	/**
	 * Sendet eine Nachricht an den RMX-Server mit folgendem Befehlsinhalt:</BR>
	 * Loksteuerung Funktionen</BR>
	 * OPCODE: 0x28</BR>
	 * Pro Funktionsbyte:</BR>
	 * Bit nicht gesetzt (0): aus</BR>
	 * Bit gesetzt (1): ein</BR>
	 * 
	 * @param trainNumber - int - Nummer der Lok, welche gesteuert werden soll
	 * @param modeF0F7 - byte - Funktionen 0 (Licht) bis 7 
	 * @param modeF8F15 - byte - Funktionen 8 bis 15 
	 * @param modeF16F23 - byte - Funktionen 16 bis 23 
	 */
	 ...
	 *            Höchstgeschwindigkeit 1 bis 7</BR>Bit 3 bis Bit 5:
	 ...
	 */
	public static void send0xc0(int adrrmx, byte cvh, byte cvl, byte mode, byte value) {
		// <0x7c><0x08><0xc0><ADRRMX><CVH><CVL><MODE><VALUE>  
		//  [0]   [1]   [2]    [3]    [4]  [5]   [6]   [7]
		
		// Rumpf formen
		byte[] byteToSend = new byte[] { Connection.HEADBYTE, 0x08, (byte) 192, (byte) adrrmx,
				cvh, cvl, mode, value };
